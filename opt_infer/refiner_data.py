...
def main():
    ...
    p.add_argument("--dump", required=True); p.add_argument("--targets", default="holdout", choices=["holdout", "test"]); p.add_argument("--test_gt_dir", default="", help="09/09: thư mục GT test (chỉ để đo trần) → dump test kèm gt_und")
    ...
    p.add_argument("--depth_mode", default="ed", choices=["ed", "quant", "cf"], help="09/09 q48: ed = kỳ vọng (gsplat ED, z TÂM Gaussian) · quant = phân vị dọc tia (median depth kiểu 2DGS/GOF)")
    p.add_argument("--depth_q", type=float, default=0.5, help="phân vị dùng khi --depth_mode quant (0,5 = median; nhỏ hơn = nghiêng về MẶT TRƯỚC)")
    p.add_argument("--depth_smooth", type=int, default=0, help="09/09 q60: >0 = số vòng làm trơn DỊ HƯỚNG bản đồ depth (theo biên render) trước khi warp — nhắm TÁN XẠ 0,38 %, thứ q49-cf chứng minh là nút thắt")
    p.add_argument("--depth_smooth_sig", type=float, default=0.04, help="ngưỡng biên của khuếch tán dị hướng (nhỏ = tôn trọng mép hơn)")
    p.add_argument("--depth_cf_k", type=float, default=1.0, help="09/09 q55: hệ số chỉnh của xấp xỉ Cornish-Fisher (1,0 = lý thuyết)")
    p.add_argument("--depth_levels", type=int, default=96, help="số bậc far_plane khi quét CDF độ sâu")
    ...
    def src_depth(si):
        ...
        if si not in depth_cache:
            ...
            elif a.depth_mode == "cf":
                d, _ = FT.render_depth_cf(splats, scene.train_metas[si].w2c(), K_np, W, H, dev, k=a.depth_cf_k)
            if a.src_depth_dir:
    ...
    def _rd_und(path):
        im = cv2.imread(path)
        if _pad: return cv2.remap(im, _ux, _uy, {"linear": cv2.INTER_LINEAR, "cubic": cv2.INTER_CUBIC, "lanczos": cv2.INTER_LANCZOS4}[os.environ.get("VT_UNDISTORT_INTERP", "linear")])   # 08/09 q8: env như dataset.py
    ...
    for name, w2c, m_t in targets:
        ...
        with torch.no_grad():
            ...
            if a.depth_mode == "quant":   # 09/09 q48: depth theo PHÂN VỊ dọc tia thay cho kỳ vọng (xem fuse_test.render_depth_quantile)
                depth_t, _ = FT.render_depth_quantile(splats, w2c, K_np, W, H, dev, q=a.depth_q, levels=a.depth_levels, ref=depth_t)
            elif a.depth_mode == "cf":    # 09/09 q55: trung vị xấp xỉ bằng mô-men bậc 3, MỘT lượt rasterize
                depth_t, _ = FT.render_depth_cf(splats, w2c, K_np, W, H, dev, k=a.depth_cf_k)
            if a.depth_smooth > 0:
                # 09/09 q60 — KHỬ TÁN XẠ của depth. Đo 09/09 (q53): median chỉ khử THIÊN VỊ (−0,33 % → −0,10 %)
                # còn tán xạ MAD ~0,38 % gần như không đổi; và q49 cf (thiên vị ≈0 nhưng tán xạ 0,50 %) rớt
                # −0,77 điểm ⇒ với việc NẮN ẢNH thì nhiễu từng pixel mới là thứ giết chết, không phải thiên vị.
                # Làm trơn DỊ HƯỚNG theo biên ảnh render (Perona-Malik, cùng hàm dfix dùng) → giữ mép nhà.
                _wt = (alpha_t > 0.5).float()
                depth_t = aniso_smooth(depth_t, _wt, R.permute(2, 0, 1) if R.dim() == 3 else R,
                                          iters=int(a.depth_smooth), lam=0.22, sig=a.depth_smooth_sig)
        ...
        if a.targets == "test" and a.test_gt_dir:   # 09/09 (q21 oracle trần data): GT test có sẵn local → dump test có GT để train refiner "cùng phân bố test"
            _c = [q for e in ("", ".JPG", ".jpg", ".png") for q in [os.path.join(a.test_gt_dir, os.path.splitext(name)[0] + e)] if os.path.isfile(q)]
            gt = _c[0] if _c else ""
